...
class IranIngestor(BaseIngestor):
    """
    Iran-specific OSINT ingestion module.
    Expand with:
      - IRGC leadership changes
      - AEOI nuclear scientists
      - Quds Force networks
      - Academic publications (Sharif University, AEOI)
    """

    def __init__(self, data_root: str = "./kpv_data"):
        super().__init__("IR", data_root)
    ...
    def fetch_raw(self) -> List[Any]:
        self.logger.info("Fetching raw OSINT data for Iran")

        raw = [
            {
                "name_native": "علی اکبر صالحی",
                "name_latin": "Ali Akbar Salehi",
                "sector": "nuclear",
                "service_or_agency": "AEOI",
                "current_position": "Former Head, Atomic Energy Organization of Iran",
                "thematic_keywords": ["nuclear engineering", "uranium enrichment"],
            }
        ]
    ...
    def resolve_identities(
        ...
        self.logger.info("Resolving identities for Iran")
        ...
        for item in normalized:
            ...
            if match:
                ...
                new_id = f"ir_person_{len(updated_persons) + 1}"
    ...
    def update_dataset(
        ...
        self.logger.info("Finalizing dataset update for Iran")
        return resolved
